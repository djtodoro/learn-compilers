...
\lstdefinelanguage{MLIR}{morekeywords={func,module,memref,scf,linalg,tensor,arith,affine,vector},sensitive=true,morecomment=[l]{//}}
...
\noindent\textit{Note: all code examples in this document have been updated for xDSL version 0.71.0. If an example does not work when you read this, check the current function and class signatures, as the xDSL API changes between releases.}

Let's create a simple IR program using xDSL:

\begin{lstlisting}[language=Python, caption=Creating IR in xDSL]
from xdsl.dialects import arith, func
from xdsl.dialects.builtin import i32, FunctionType, ModuleOp
from xdsl.builder import ImplicitBuilder

# 1) an empty module
module = ModuleOp([])

# 2) open the module body and build inside it
with ImplicitBuilder(module.body):
    # Create function signature: (i32, i32) -> i32
    func_type = FunctionType.from_lists([i32, i32], [i32])
    add = func.FuncOp("add", func_type)

    # 3) open the function body; `as (...)` yields the block arguments
    with ImplicitBuilder(add.body) as (arg0, arg1):
        # Create add operation
        result = arith.AddiOp(arg0, arg1)
        # Return the result
...
def infer_matmul_type(lhs: SSAValue, rhs: SSAValue) -> TensorType:
    m, _ = lhs.type.get_shape()      # (M, K)
    _, n = rhs.type.get_shape()      # (K, N)
    return TensorType(lhs.type.element_type, [m, n])   # -> (M, N)
...
class MatMulOp(IRDLOperation):
    ...
    def verify_(self):
        ...
        if lhs_shape[-1] != rhs_shape[0]:
            raise VerifyException("Incompatible matrix dimensions")
\end{lstlisting}
...
add_op = arith.AddiOp(a, b)       # result = a + b
mul_op = arith.MuliOp(a, b)       # result = a * b

# Memory operations
alloc_op = memref.AllocOp([], [], MemRefType(f32, [10, 20]))  # allocate 10x20 array
load_op  = memref.LoadOp.get(memref_value, indices)          # load from memory
store_op = memref.StoreOp.get(value, memref_value, indices)  # store to memory

# Control flow operations
for_op = scf.ForOp(lower_bound, upper_bound, step, [])       # for loop
if_op  = scf.IfOp(condition, [result_types], true_region)    # if-then-else
\end{lstlisting}
...
const = arith.ConstantOp.from_int_and_width(42, 32)  # 42 is an attribute

# Add has operands (runtime values)
result = arith.AddiOp(x, y)  # x and y are operands

# Alloc has attributes (shape) but no data operands
mem = memref.AllocOp([], [], MemRefType(f32, [100]))  # shape is attribute
\end{lstlisting}
...
class OperationCounterPass(ModulePass):
...
class UseDefAnalysis(ModulePass):
...
class DominanceAnalysis(ModulePass):
    ...
    def apply(self, ctx: Context, module: builtin.ModuleOp) -> None:
        ...
        for func_op in module.body.ops:
            if isinstance(func_op, func.FuncOp):
                # DominanceInfo works on a region and answers block-level
                # dominance queries: dominates(a, b) / strictly_dominates(a, b).
                dom = DominanceInfo(func_op.body)
                blocks = list(func_op.body.blocks)
                names = {b: f"bb{i}" for i, b in enumerate(blocks)}

                # For each block, list the blocks that dominate it
                for b in blocks:
                    dominators = [names[a] for a in blocks if dom.dominates(a, b)]
                    print(f"Block {names[b]} dominated by: {dominators}")
\end{lstlisting}
...
class ConstantPropagationAnalysis(ModulePass):
    ...
    def apply(self, ctx: Context, module: builtin.ModuleOp) -> None:
        ...
        # (op.result is used as an SSA-value key; op.value.value.data is the int)
        for op in module.walk():
            if isinstance(op, arith.ConstantOp):
                known_constants[op.result] = op.value.value.data
...
class FoldAddZero(RewritePattern):
    ...
    @op_type_rewrite_pattern
    def match_and_rewrite(self, op: arith.AddiOp,
                          rewriter: PatternRewriter) -> None:
        # Check if right operand is zero
        if isinstance(op.rhs.owner, arith.ConstantOp):
            if op.rhs.owner.value.value.data == 0:
                # Replace all uses of the add with the left operand
                rewriter.replace_all_uses_with(op.result, op.lhs)
                rewriter.erase(op)
                return

        # Check if left operand is zero
        if isinstance(op.lhs.owner, arith.ConstantOp):
            if op.lhs.owner.value.value.data == 0:
                rewriter.replace_all_uses_with(op.result, op.rhs)
                rewriter.erase(op)
\end{lstlisting}
...
class DeadCodeElimination(ModulePass):
    ...
    def apply(self, ctx: Context, module: builtin.ModuleOp) -> None:
        # Iterate to a fixpoint: removing a dead op can make its operands dead.
        changed = True
        while changed:
            changed = False
            ops_to_remove = []
            ...
            for op in module.walk():
                ...
                # IMPORTANT: only consider ops that actually PRODUCE a value.
                # all(...) over an empty result list is True, so without this
                # guard we would wrongly delete result-less ops such as
                # func.return, func.func and the module itself.
                if not op.results:
                    continue
 
                # Dead if none of its results are used
    ...
    def has_side_effects(self, op: Operation) -> bool:
        ...
        return isinstance(op, (memref.StoreOp, func.CallOp))
\end{lstlisting}
...
class CommonSubexpressionElimination(ModulePass):
    ...
    def apply(self, ctx: Context, module: builtin.ModuleOp) -> None:
        # Map from expression key to the SSA value that first produced it.
        expression_map = {}

        for op in list(module.walk()):
            if self.is_pure(op):
                # Key for this expression
                key = self.get_expression_key(op)

                if key in expression_map:
                    # Duplicate found: reuse the earlier result, drop this op.
                    # NB: in straight-line code the first occurrence dominates
                    # later ones; a general CSE would check dominance here.
                    op.results[0].replace_all_uses_with(expression_map[key])
                    op.detach()
                    op.erase()
                else:
                    expression_map[key] = op.results[0]
    
    def is_pure(self, op: Operation) -> bool:
        # Only fold value-producing ops with no side effects.
        return bool(op.results) and op.has_trait(Pure)
    
    def get_expression_key(self, op: Operation):
        """Create a hashable key for the expression.

        Must include attributes AND properties, not just operands: otherwise
        two different constants (e.g. `constant 5` and `constant 7`, both with
        no operands) would collide and be wrongly merged.
        """
        return (op.name,
                tuple(op.operands),
                tuple(sorted(op.attributes.items())),
                tuple(sorted(op.properties.items())))
\end{lstlisting}
...
class LoopUnrolling(ModulePass):
    """Unroll small loops"""
    ...
    def should_unroll(self, loop: scf.ForOp) -> bool:
        """Decide if loop should be unrolled"""
        # Get loop bounds if constant
        if (isinstance(loop.lb.owner, arith.ConstantOp) and
                isinstance(loop.ub.owner, arith.ConstantOp)):
            lb = loop.lb.owner.value.value.data
            ub = loop.ub.owner.value.value.data
            iterations = ub - lb
    ...
    def unroll_loop(self, loop: scf.ForOp):
...
from xdsl.context import Context
from xdsl.passes import ModulePass
from xdsl.dialects import builtin, arith, memref, scf
from xdsl.dialects.builtin import (MemRefType, f32, IndexType,
    IntegerAttr, FloatAttr)
from xdsl.builder import ImplicitBuilder, Builder, InsertPoint

class LowerMatrixOps(ModulePass):
    """Lower matrix operations to loops.

    Assumes a memref-based MatMulOp: (MxK) @ (KxN) -> (MxN).
    """
    ...
    def lower_matmul(self, matmul: MatMulOp):
        ...
        A, B = matmul.lhs, matmul.rhs
 
        # Get dimensions
        m, k = A.type.get_shape()
        _, n = B.type.get_shape()
        idx = IndexType()

        # Build the loop nest just before the matmul op
        with ImplicitBuilder(Builder(InsertPoint.before(matmul))):
            c0 = arith.ConstantOp(IntegerAttr(0, idx))
            c1 = arith.ConstantOp(IntegerAttr(1, idx))
            cM = arith.ConstantOp(IntegerAttr(m, idx))
            cN = arith.ConstantOp(IntegerAttr(n, idx))
            cK = arith.ConstantOp(IntegerAttr(k, idx))
            zero = arith.ConstantOp(FloatAttr(0.0, f32))

            # Allocate result
            C = memref.AllocOp.get(matmul.result.type.element_type,
                                   shape=[m, n])
            ...
            def i_body(i_args):
                ...
                def j_body(j_args):
                    ...
                    # Inner loop over k with an accumulator (iter_arg)
                    @Builder.implicit_region((idx, f32))
                    def k_body(k_args):
                        (kk, acc) = k_args
                        a_elem = memref.LoadOp.get(A, [i, kk])
                        b_elem = memref.LoadOp.get(B, [kk, j])
                        prod = arith.MulfOp(a_elem, b_elem)
                        new_acc = arith.AddfOp(acc, prod)
                        scf.YieldOp(new_acc)
                    
                    k_loop = scf.ForOp(c0, cK, c1, [zero], k_body)
                    # Store the accumulated result
                    memref.StoreOp.get(k_loop.results[0], C, [i, j])
                    scf.YieldOp()
                
                scf.ForOp(c0, cN, c1, [], j_body)
                scf.YieldOp()

            scf.ForOp(c0, cM, c1, [], i_body)
        
        # Replace matmul with the lowered result buffer, then remove it
        matmul.result.replace_all_uses_with(C.results[0])
        matmul.detach()
        matmul.erase()
\end{lstlisting}
...
# IR Generator
class ExprIRGen:
    def compile(self, expr: Expr) -> SSAValue:
...
class FuseBatchNormIntoConv(ModulePass):
    ...
    def fuse(self, conv, bn):
        ...
        bn.result.replace_all_uses_with(fused_conv.result)
        conv.erase()
        bn.erase()
\end{lstlisting}
...
# Illustrative: defines an invented `ml` dialect to sketch fusion + lowering.
from xdsl.ir import Dialect
from xdsl.irdl import irdl_op_definition, IRDLOperation, operand_def, result_def
from xdsl.dialects.builtin import TensorType
from xdsl.pattern_rewriter import (
    RewritePattern, PatternRewriter, op_type_rewrite_pattern,
)
...
MLDialect = Dialect("ml", [LinearOp, ReluOp])
...
class FuseLinearRelu(RewritePattern):
    ...
    def match_and_rewrite(self, relu: ReluOp,
        ...
        # Check if input is from a linear layer
        if isinstance(relu.input.owner, LinearOp):
            ...
            # Replace both ops with the fused version
            rewriter.replace_all_uses_with(relu.output, fused.output)
            rewriter.erase(linear)

# Lower to hardware operations
class LowerMLToHardware(ModulePass):
...
class AutotuningPass(ModulePass):
...
class StoreLoadForwarding(ModulePass):
    ...
    def apply(self, ctx: Context, module: builtin.ModuleOp) -> None:
        ...
        #     load.res.replace_all_uses_with(stored_value)
        #     load.detach(); load.erase()
...
from xdsl.ir import Dialect, SSAValue, TypeAttribute, ParametrizedAttribute
from xdsl.irdl import (irdl_op_definition, irdl_attr_definition,
                       IRDLOperation, operand_def, result_def, traits_def)
from xdsl.traits import Pure
...
@irdl_attr_definition
class QuantumRegisterType(ParametrizedAttribute, TypeAttribute):
...
class HadamardOp(IRDLOperation):
    ...
    input = operand_def(QubitType)
    result = result_def(QubitType)
    traits = traits_def(Pure())   # no side effects => DCE may remove it

    def __init__(self, inp: SSAValue):
        super().__init__(operands=[inp], result_types=[QubitType()])
...
QuantumDialect = Dialect("quantum",
                         [HadamardOp, CNOTOp, MeasureOp],
                         [QubitType, QuantumRegisterType])

# TODO: Optimization passes
class QuantumCircuitOptimizer(ModulePass):
    ...
    def apply(self, ctx, module):
        ...
        # - Gate cancellation (H H = I): match a HadamardOp whose input is
        #   another HadamardOp, replace its result's uses with the inner
        #   input, and erase it.
        # - Gate commutation
...
class Vectorization(ModulePass):
    ...
    def is_vectorizable(self, loop: scf.ForOp) -> bool:
    ...
    def vectorize_loop(self, loop: scf.ForOp):
    ...
    def analyze_dependencies(self, loop: scf.ForOp) -> bool:
...
# Illustrative: invented `finance` dialect.
from xdsl.ir import Dialect
from xdsl.irdl import irdl_op_definition, IRDLOperation, operand_def, result_def
...
FinanceDialect = Dialect("finance", [BlackScholesOp])

# Optimize for numerical stability
class NumericalStabilityPass(ModulePass):
...
# Illustrative: invented `dist` dialect.
from xdsl.ir import Dialect
from xdsl.irdl import (irdl_op_definition, IRDLOperation,
                       operand_def, result_def, attr_def)
from xdsl.dialects.builtin import TensorType, StringAttr
...
class AllReduceOp(IRDLOperation):
    ...
    input = operand_def(TensorType)
    output = result_def(TensorType)
    reduction = attr_def(StringAttr)  # "sum", "max", etc.

DistributedDialect = Dialect("dist", [AllReduceOp])

class DistributionPass(ModulePass):
...
from xdsl.ir import Dialect, SSAValue
from xdsl.irdl import (irdl_op_definition, IRDLOperation,
                       operand_def, result_def, traits_def)
from xdsl.traits import Pure
from xdsl.dialects.builtin import TensorType
from xdsl.utils.exceptions import VerifyException
...
class MatMulOp(IRDLOperation):
    ...
    # Shape inference: (m x n) @ (n x p) -> (m x p)
    def verify_(self):
        # Check dimension compatibility
        if self.lhs.type.get_shape()[-1] != self.rhs.type.get_shape()[0]:
            raise VerifyException("incompatible matmul dimensions")
...
class TransposeOp(IRDLOperation):
    ...
    input = operand_def(TensorType)
    result = result_def(TensorType)
    traits = traits_def(Pure())   # pure => DCE may remove dead transposes

    # Shape transformation: (m x n) -> (n x m)
    # Key for optimization patterns
    def __init__(self, inp:x
...
class MatrixIRGenerator:
    ...
    def generate_operation(self, op_info):
        ...
        if op_type == 'matmul':
            ...
            # %tmp = matrix.transpose %c : (m x n) -> (n x m)
            # %d = matrix.transpose %tmp : (n x m) -> (m x n)
...
from xdsl.pattern_rewriter import (
    PatternRewriter, RewritePattern, op_type_rewrite_pattern,
)
...
class DoubleTransposeElimination(RewritePattern):
    ...
    def match_and_rewrite(self, op: TransposeOp, rewriter: PatternRewriter):
        ...
        if isinstance(op.input.owner, TransposeOp):
            ...
            # Replace double transpose with the original value
            rewriter.replace_all_uses_with(op.result, original)
            rewriter.erase(op)
